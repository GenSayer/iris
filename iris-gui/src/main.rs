...
impl App {
    ...
    fn snap_window_to_fb(ctx: &egui::Context, fb_px: egui::Vec2, central_avail: egui::Vec2) {
        ...
        // Native scale = 1 framebuffer pixel : 1 logical point — an integer
        // device-pixel scale (1× standard, 2× HiDPI) so the picture stays crisp.
        // In egui's zoom-scaled points that is 1/zoom points per pixel.
        let native = 1.0 / ctx.zoom_factor().max(0.1);
        // Prefer native; shrink to fit the monitor, leaving ~10% for the menu
        // bar / dock / taskbar so the window doesn't tuck under them.
        let mut scale = native;
        if let Some(m) = ctx.input(|i| i.viewport().monitor_size) {
            const MARGIN: f32 = 0.90;
            let max_fb_w = (m.x * MARGIN - chrome_w).max(64.0);
            let max_fb_h = (m.y * MARGIN - chrome_h).max(64.0);
            scale = scale.min(max_fb_w / fb_px.x).min(max_fb_h / fb_px.y);
        }
        scale = scale.clamp(0.05, native);
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.emu.is_running() {
                ...
                {
                    Self::snap_window_to_fb(ui.ctx(), egui::vec2(1280.0, 1024.0), ui.available_size());
                    self.pending_launcher_fit = false;
                }
