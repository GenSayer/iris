...
impl App {
    ...
    fn run_state_label(&mut self, ui: &mut egui::Ui) {
        ...
        // When the machine was launched on the PCAP backend, surface that next to
        // the NAT-centric NET indicator above so the active backend is verifiable
        // (and the indicator's NAT semantics aren't misread). Only shown on a
        // PCAP build that actually started in PCAP mode.
        if running {
            if let Some((iris::config::NetMode::Pcap, iface)) = self.launched_net.as_ref() {
